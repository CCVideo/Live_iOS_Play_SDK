...
- (void)startWithDownloadModel:(CCDownloadModel *)downloadModel
{
    ...
    if (downloadModel.state == DWDownloadStateReadying) {
    ...
    if (task && task.state == NSURLSessionTaskStateRunning) {
        downloadModel.state = DWDownloadStateRunning;
        [self downloadModel:downloadModel error:nil];
        return;
    }
...
- (void)suspendWithDownloadModel:(CCDownloadModel *)downloadModel
{
    ...
    downloadModel.state = DWDownloadStateSuspended;
...
//        NSLog(@"取消，并且删除了数据");
    ...
    if (error && resumeData) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^(){
            downloadModel.state = DWDownloadStateFailed;
        ...
        //下载完成
        dispatch_async(dispatch_get_main_queue(), ^(){
            downloadModel.state = DWDownloadStateCompleted;
            [self downloadModel:downloadModel error:nil];
...
- (BOOL)canResumeDownlaodModel:(CCDownloadModel *)downloadModel
{
    ...
    @synchronized (self) {
        ...
        for (CCDownloadModel * model in self.downloadModels) {
            if (model.state == DWDownloadStateRunning) {
                downloadingCount++;
            }
        }
        
        if (downloadingCount >= _maxDownloadCount) {
            downloadModel.state = DWDownloadStateReadying;
            [self downloadModel:downloadModel error:nil];
    ...
    @synchronized (self) {
        ...
        if (_resumeDownloadFIFO) {
            ...
            [self.downloadModels enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                if (((CCDownloadModel *)obj).state == DWDownloadStateReadying) {
