...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([dialog.fromuserrole isEqualToString:@"publisher"]) {//主讲
        str = @"lecturer_nor";
        headImgName = @"chatHead_lecturer";
    } else if ([dialog.fromuserrole isEqualToString:@"student"]) {//学生或观众
        str = @"role_floorplan";
        headImgName = @"chatHead_student";
    } else if ([dialog.fromuserrole isEqualToString:@"host"]) {//主持人
        str = @"compere_nor";
        headImgName = @"chatHead_compere";
    } else if ([dialog.fromuserrole isEqualToString:@"unknow"]) {//其他没有角色
        str = @"role_floorplan";
        headImgName = [NSString stringWithFormat:@"用户%d", arc4random_uniform(5) + 1];
    } else if ([dialog.fromuserrole isEqualToString:@"teacher"]) {//助教
        str = @"assistant_nor";
        headImgName = @"chatHead_assistant";
    }
    head.image = [UIImage imageNamed:headImgName];
...
-(CustomTextField *)chatTextField {
    if(!_chatTextField) {
        ...
        _chatTextField.placeholder = @"请输入消息";
