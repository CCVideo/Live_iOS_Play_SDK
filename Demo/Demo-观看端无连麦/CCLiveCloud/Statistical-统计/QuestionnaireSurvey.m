...
-(void)initUI {
    ...
    if(!self.isScreenLandScape) {//竖屏约束
        [_view mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.mas_equalTo(ws).offset(CCGetRealFromPt(20));
            make.right.mas_equalTo(ws).offset(-CCGetRealFromPt(20));
            make.top.mas_equalTo(ws).offset(CCGetRealFromPt(247));
            make.bottom.mas_equalTo(ws).offset(-CCGetRealFromPt(72) - kScreenBottom);
        }];
    } else {//横屏约束
        [_view mas_makeConstraints:^(MASConstraintMaker *make) {
    ...
    [_scrollView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.mas_equalTo(ws.view);
        make.right.mas_equalTo(ws.view);
        make.top.mas_equalTo(ws.view).offset(CCGetRealFromPt(83));
    ...
    [contentLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.top.mas_equalTo(ws.scrollView);
    ...
    for(int i = 0;i < [subjectsArray count];i++) {
        ...
        [leftLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.mas_equalTo(ws.scrollView.mas_left).offset(CCGetRealFromPt(30));
            if(i == 0) {
        ...
        [centerLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.mas_equalTo(ws.scrollView.mas_left).offset(CCGetRealFromPt(30) + leftSizeNum.width + CCGetRealFromPt(15));
        ...
        [centerImageView mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.mas_equalTo(ws.scrollView.mas_left).offset(CCGetRealFromPt(30) + leftSizeNum.width + CCGetRealFromPt(15) + centerSizeNum.width + CCGetRealFromPt(10));
        ...
        [contentLabelQue mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.mas_equalTo(ws.scrollView.mas_left).offset(CCGetRealFromPt(30) + leftSizeNum.width + CCGetRealFromPt(15) + centerSizeNum.width + CCGetRealFromPt(10) + centerImageView.frame.size.width + CCGetRealFromPt(10) + 1);
        ...
        if(([opthionsDic[@"type"] intValue] == 0 || [opthionsDic[@"type"] intValue] == 1) && optionsArray) {
            for(int j = 0;j < [optionsArray count];j++) {
                ...
                [selectButton mas_makeConstraints:^(MASConstraintMaker *make) {
                    make.left.mas_equalTo(ws.scrollView.mas_left).offset(CCGetRealFromPt(74));
                    if(j == 0) {
...
-(UILabel *)titleLabel {
    if(!_titleLabel) {
        ...
        _titleLabel.text = self.isStastic ? @"问卷统计": @"问卷";
