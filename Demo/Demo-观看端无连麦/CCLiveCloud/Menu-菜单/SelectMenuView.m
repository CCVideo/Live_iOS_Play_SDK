...
-(void)hiddenAllBtns:(BOOL)hidden{
    if (hidden) {//收回菜单
        [UIView animateKeyframesWithDuration:0.3 delay:0 options:UIViewKeyframeAnimationOptionBeginFromCurrentState animations:^{
            self.alpha = 0.1f;
            self.frame = CGRectMake(self.frame.origin.x, self.frame.origin.y + CCGetRealFromPt(326) - 50, CCGetRealFromPt(70), CCGetRealFromPt(70));
            self.menuBtn.frame = CGRectMake(-4, -4, CCGetRealFromPt(86), CCGetRealFromPt(86));
        ...
        [UIView animateKeyframesWithDuration:0.3 delay:0 options:UIViewKeyframeAnimationOptionBeginFromCurrentState animations:^{
            self.alpha = 1.f;
            self.frame = CGRectMake(self.frame.origin.x, self.frame.origin.y - CCGetRealFromPt(326) + 50, CCGetRealFromPt(70), CCGetRealFromPt(410) - 8 - 50);
            self.menuBtn.frame = CGRectMake(-4, CCGetRealFromPt(326) - 4 - 50, CCGetRealFromPt(86), CCGetRealFromPt(86));
            [self updateInformationViewFrame];
        } completion:^(BOOL finished) {
