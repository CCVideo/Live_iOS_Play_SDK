    ...
    if (!StrNotEmpty(result) || rangeRoomId.location == NSNotFound || rangeUserId.location == NSNotFound || (self.index == 3 && rangeLiveId.location == NSNotFound)) {
        ...
        if(self.index == 1) {
            ...
            if(rangeRecordId.location == NSNotFound) {
                ...
                NSString *liveId = [result substringFromIndex:rangeLiveId.location + rangeLiveId.length];
//                NSString *liveId = [result substringWithRange:NSMakeRange(rangeLiveId.location + rangeLiveId.length, rangeRecordId.location - 1 - (rangeLiveId.location + rangeLiveId.length))];
//                NSString *recordId = [result substringFromIndex:rangeRecordId.location + rangeRecordId.length];
