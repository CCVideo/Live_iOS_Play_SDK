...
- (void) UIControlEventTouchDown:(UISlider *)sender {
    UIImage *image = [UIImage imageNamed:@"进度条"];//图片模式，不设置的话会被压缩
    [_slider setThumbImage:image forState:UIControlStateNormal];//设置图片
}

- (void) durationSliderDone:(UISlider *)sender
{
    UIImage *image2 = [UIImage imageNamed:@"进度条"];//图片模式，不设置的话会被压缩
...
- (void)setupUI {
    ...
    topShadow.image = [UIImage imageNamed:@"播放器Bar反"];
    ...
    bottomShadow.image = [UIImage imageNamed:@"播放器Bar"];
    ...
    [self.pauseButton setImage:[UIImage imageNamed:@"暂停"] forState:UIControlStateNormal];
    [self.pauseButton setImage:[UIImage imageNamed:@"播放"] forState:UIControlStateSelected];
    ...
    //设置滑块的颜色
    [_slider setThumbImage:[UIImage imageNamed:@"进度条"] forState:UIControlStateNormal];
    //对滑动条添加事件函数
    ...
    [self.quanpingButton setImage:[UIImage imageNamed:@"展开"] forState:UIControlStateNormal];
    [self.quanpingButton setImage:[UIImage imageNamed:@"收缩"] forState:UIControlStateSelected];
...
//    self.liveUnStart.image = [UIImage imageNamed:@"直播未开始bg"];
...
//    alarmClock.image = [UIImage imageNamed:@"直播未开始"];
